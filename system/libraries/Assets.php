...
//Library to manage assets (An asset is a pack of one or more CSS/JS files) with CDN support

class CI_Assets {

	private $CI;
	private $hosting = ASSETS;
	private $assets; //All possible assets (see application/config/assets.php)
	private $css = array();
	private $cdn = array();
	private $js = array();
	...
	public function __construct($assets = array())
	{
		...
		elseif(count($assets))
		{
			...
			$this->assets = $assets; //No need to load config. CI already did it for us
		}
		else
		{
	...
	public function add($asset)
	{
		...
		{
			if(isset($this->assets[$asset])) //The asset exist
			{
	...
	public function build_js()
	{
		...
		{
			$output .= "<!-- Load from CDN with fallback to local -->\n";
